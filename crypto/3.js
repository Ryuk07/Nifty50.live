...
function loadProducts() {
  ...
  for (var i = 0; i < res.length; i++) {
    ...
    if (parts[1] != "EUR" && parts[1] != "GBP") {
      ...
      else products[parts[1]].push(parts[0] + "-" + parts[1]);
    }
  }
  ...
  for (var k in products) {
    ...
    for (var i = 0; i < products[k].length; i++) {
      ...
      url +=
        "%7B%22s%22%3A%22COINBASE%3A" + product.replace("-", "") + "%22%7D";
  ...
  url +=
    "%5D%2C%22utm_source%22%3A%22%22%2C%22utm_medium%22%3A%22widget%22%2C%22utm_campaign%22%3A%22market-overview%22%7D";
...
function loadDropbox(id) {
  ...
  var line0 =
    '<option value="" selected="selected" disabled hidden>Chart ' +
    id.split("_")[1] +
    "</option>";
